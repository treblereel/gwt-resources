...
public class CurrencyList implements Iterable<CurrencyData> {
  ...
  /**
   * Add all entries in {@code override} to the original map, replacing any existing entries. This
   * is used by subclasses that need to slightly alter the data used by the parent locale.
   */
  ...
  private static void loadCurrencyValuesNative(
  ...
  /**
   * JS map of currency codes to CurrencyData objects. Each currency code is assumed to be a valid
   * JS object key.
   */
  protected JavaScriptObject dataMapNative;

  /**
   * Map of currency codes to localized currency names. This is kept separate from {@link
   * #dataMapJava} above so that the names can be completely removed by the compiler if they are not
   * used.
   */
  protected HashMap<String, String> namesMapJava;

  /**
   * JS map of currency codes to localized currency names. This is kept separate from {@link
   * #dataMapNative} above so that the names can be completely removed by the compiler if they are
   * not used. Each currency code is assumed to be a valid JS object key.
   */
