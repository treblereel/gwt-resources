 ...
 *
 * <p>Implementors should subclass {@link DefaultDateTimeFormatInfo} so when methods are added they
 * will get reasonable defaults and not break. See the same class for example values returned by
 * these methods.
 */
public interface DateTimeFormatInfo {
  ...
  /**
   * Returns a date/time format from a date format pattern and a time format pattern, using the
   * locale default joining.
   *
  ...
  /**
   * Returns a date/time format from a date format pattern and a time format pattern, using "full"
   * joining.
   *
   * @param timePattern the time pattern String
   * @param datePattern the data pattern String
  ...
  /**
   * Returns the day which generally comes first in a weekly calendar view, as an index into the
   * return value of {@link #weekdaysFull()}.
   */
  ...
  /**
   * Returns an array of the shortest abbreviations for months, typically a single character and not
   * guaranteed to be unique.
   */
  String[] monthsNarrow();

  /**
   * Returns an array of the shortest abbreviations for months suitable for use in a stand-alone
   * context, typically a single character and not guaranteed to be unique.
   */
  ...
  /**
   * Returns an array of the shortest abbreviations for weekdays, typically a single character and
   * not guaranteed to be unique.
   */
  String[] weekdaysNarrow();

  /**
   * Returns an array of the shortest abbreviations for weekdays suitable for use in a stand-alone
   * context, typically a single character and not guaranteed to be unique.
   */
  ...
  /**
   * Returns the day which ends the weekend, as an index into the return value of {@link
   * #weekdaysFull()}.
   *
   * <p>Note that this value may be numerically less than {@link #weekendStart()} - for example,
   * {@link #weekendStart()} of 6 and {@link #weekendEnd()} of 0 means Saturday and Sunday are the
   * weekend.
   */
  int weekendEnd();

  /**
   * Returns the day which starts the weekend, as an index into the return value of {@link
   * #weekdaysFull()}.
   */
  int weekendStart();
}
