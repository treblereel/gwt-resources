...
public class FirstStrongDirectionEstimator extends DirectionEstimator {
  ...
  /** An instance of FirstStrongDirectionEstimator, to be returned by {@link #get}. */
  private static final FirstStrongDirectionEstimator instance = new FirstStrongDirectionEstimator();
  ...
  /**
   * Estimates the direction of a given string using the "first strong" heuristic: The return value
   * is determined by the first character in the string with strong directionality. If there is no
   * such character, the return value is DEFAULT.
   *
  ...
  public Direction estimateDirection(String str) {
    return BidiUtils.get().startsWithRtl(str)
        ? Direction.RTL
