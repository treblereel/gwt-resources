...
/**
 * A wrapper to {@link BidiFormatter} whose methods return {@code SafeHtml} instead of {@code
 * String}.
 */
public class SafeHtmlBidiFormatter extends BidiFormatterBase {
  ...
  static class Factory extends BidiFormatterBase.Factory<SafeHtmlBidiFormatter> {
    @Override
    public SafeHtmlBidiFormatter createInstance(Direction contextDir, boolean alwaysSpan) {
  ...
  /**
   * Factory for creating an instance of SafeHtmlBidiFormatter given the context direction. The
  ...
  /**
   * Factory for creating an instance of SafeHtmlBidiFormatter given the context direction and the
   ...
   *     structure of the output does not depend on the combination of directions
   */
  public static SafeHtmlBidiFormatter getInstance(Direction contextDir, boolean alwaysSpan) {
    return factory.getInstance(contextDir, alwaysSpan);
  }

  /**
   * Factory for creating an instance of SafeHtmlBidiFormatter whose context direction matches the
   * current locale's direction. The default behavior of {@link #spanWrap} and its variations is set
   * to avoid span wrapping unless it's necessary ('dir' attribute needs to be set).
   */
  ...
  /**
   * Factory for creating an instance of SafeHtmlBidiFormatter whose context direction matches the
   * current locale's direction, and given the desired span wrapping behavior (see below).
   *
   * @param alwaysSpan Whether {@link #spanWrap} (and its variations) should always use a 'span'
   *     tag, even when the input direction is neutral or matches the context, so that the DOM
   *     structure of the output does not depend on the combination of directions
   ...
   *     when needed, to prevent the possible garbling of whatever may follow {@code html}
   * @return Input html after applying the above processing.
   */
  public SafeHtml spanWrap(SafeHtml html, boolean dirReset) {
    return SafeHtmlUtils.fromTrustedString(spanWrapBase(html.asString(), true, dirReset));
  }
  ...
  /**
   * @see BidiFormatter#spanWrapWithKnownDir( org.gwtproject.i18n.client.HasDirection.Direction,
   *     String, boolean)
   ...
   * @return Input html after applying the above processing.
   */
  public SafeHtml spanWrapWithKnownDir(Direction dir, SafeHtml html, boolean dirReset) {
  ...
  public SafeHtml spanWrapWithKnownDir(Direction dir, String str, boolean dirReset) {
    ...
    return SafeHtmlUtils.fromTrustedString(spanWrapWithKnownDirBase(dir, str, false, dirReset));
  }
  ...
  public SafeHtml unicodeWrap(SafeHtml html, boolean dirReset) {
    return SafeHtmlUtils.fromTrustedString(unicodeWrapBase(html.asString(), true, dirReset));
  }
  ...
  /**
   * @see BidiFormatter#unicodeWrapWithKnownDir( org.gwtproject.i18n.client.HasDirection.Direction,
   *     String, boolean)
   * @param dir {@code html}'s direction
   * @param html The input html
   * @return Input html after applying the above processing.
   ...
   * @return Input html after applying the above processing.
   */
  public SafeHtml unicodeWrapWithKnownDir(Direction dir, SafeHtml html, boolean dirReset) {
    return SafeHtmlUtils.fromTrustedString(
   ...
   * @return Input string after applying the above processing.
   */
  public SafeHtml unicodeWrapWithKnownDir(Direction dir, String str, boolean dirReset) {
    /*
     * unicodeWrapWithKnownDirBase does not HTML-escape, so its return value is
     * not trusted.
     */
    return SafeHtmlUtils.fromString(unicodeWrapWithKnownDirBase(dir, str, false, dirReset));
  }

  /**
   * Converts an input string into a SafeHtml. Input String must be safe (see {@link
   * com.google.gwt.safehtml.shared.SafeHtml}). Implementation: first, tries to find the input in
   * the static map, {@link #cachedSafeHtmlValues}. If not found, creates a SafeHtml instance for
   * the string and adds it to the map.
   *
   * @param str String to search for. Must be safe (see {@link
