...
public class Css2Gss {
  ...
  public Css2Gss(
      URL url,
      TreeLogger logger,
      boolean lenientConversion,
      Predicate<String> simpleBooleanConditionPredicate) {
    this(url, logger, lenientConversion, simpleBooleanConditionPredicate, new HashSet<>());
  }
  ...
  private static String convertFile(
      File resource, Set<String> simpleBooleanConditions, Set<URL> scopeFiles)
    ...
    return new Css2Gss(resource.toURI().toURL(), false, simpleConditionPredicate, scopeFiles)
        .toGss();
  }
  ...
  public String toGss() throws UnableToCompleteException {
    try {
      ...
      new UndefinedConstantVisitor(
              new HashSet<String>(defNameMapping.values()), lenient, treeLogger)
          .accept(sheet);
      ...
      GssGenerationVisitor gssGenerationVisitor =
          new GssGenerationVisitor(
              new DefaultTextOutput(false),
              defNameMapping,
              lenient,
              treeLogger,
  ...
  private static void printUsage() {
    ...
    System.err.println(
        " -r -> Recursively convert all css files on the given directory"
            + "(leaves .css files in place)");
    System.err.println(
        " -condition list_of_condition -> Specify a comma-separated list of "
            ...
            + "conditions");
    System.err.println(
        " -scope list_of_files -> Specify a comma-separated list of "
            + "css files to be used in this conversion to determine all defined variables");
  }

  /**
   * GSS allows only uppercase letters and numbers for a name of the constant. The constants need to
   * be renamed in order to be compatible with GSS. This method returns a mapping between the old
   * name and the new name compatible with GSS.
   */
  ...
  static class Options {
    ...
    static {
      ...
      argumentConsumers.put(
          "-r",
          new ArgumentConsumer() {
            @Override
      ...
      argumentConsumers.put(
          ...
          new ArgumentConsumer() {
            ...
            public boolean consume(Options option, String nextArg) {
              if (nextArg == null) {
                quitEarly(
                    "-condition option must be followed by a comma separated list of conditions");
              }

              option.simpleBooleanConditions =
