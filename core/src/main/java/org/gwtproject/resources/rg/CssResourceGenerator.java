 ...
 * Created by treblereel 11/15/18
 */
public class CssResourceGenerator extends AbstractCssResourceGenerator {
     ...
     * <p>
     * Note that the character ESERVED_IDENT_CHAR is intentionally
        ...
        for (Map.Entry<Integer, List<CssSubstitution>> entry : v.getSubstitutionPositions().entrySet()) {
            ...
            for (CssSubstitution x : entry.getValue()) {
                ...
                if (x instanceof CssIf) {
                    ...
                    validateValue(loopLogger, context.getClientBundleType(), property.getValues(), types, elements);
                    // (expr) +
    ...
    private static void validateValue(TreeLogger logger,
                                      TypeElement resourceBundleType, Value value, Types types, Elements elements)
        ...
        if (dot != null) {
            try {
                ...
                ResourceGeneratorUtil.getMethodByPath(resourceBundleType, dot.getParts(), null, types, elements);
     ...
     */
    protected static void outputCssMapArtifact(TreeLogger logger, ResourceContext context, ExecutableElement method,
        ...
        String qualifiedMethodName = bundleType.getQualifiedName() + "." + method.getSimpleName().toString();
        ...
        try {
            for (Map.Entry<ExecutableElement, String> replacement : actualReplacements.entrySet()) {
                ...
                String qualifiedName = ((TypeElement) replacement.getKey().getEnclosingElement()).getQualifiedName().toString();
    ...
    public String createAssignment(TreeLogger logger, ResourceContext context, ExecutableElement method) throws UnableToCompleteException {
        ...
        outputAdditionalArtifacts(logger, context, method, actualReplacements,
                MoreTypes.asTypeElement(cssResourceSubtype), stylesheet);
        String result = getResourceImplAsString(logger, context, method, actualReplacements,
                MoreTypes.asTypeElement(cssResourceSubtype), stylesheet);
        return result;
    }
    ...
    protected String getResourceImplAsString(TreeLogger logger, ResourceContext context,
        ...
        sw.println("new " + ((TypeElement) MoreTypes.asElement(method.getReturnType())).getQualifiedName()
                + "() {");
        sw.indent();
...
/*        List<Element> methodsList = cssResourceSubtype.getEnclosedElements().stream().
        ...
        Set<ExecutableElement> methodsSet = MoreElements.getLocalAndInheritedMethods(cssResourceSubtype, type, elements);
    ...
    protected void writeUserMethods(TreeLogger logger, SourceWriter sw,
                                    CssStylesheet sheet, ExecutableElement[] methods,
        ...
        for (ExecutableElement toImplement : methods) {
            ...
            } else if (toImplement.getReturnType().toString()
    ...
    private void writeDefAssignment(TreeLogger logger, SourceWriter sw,
                                    ExecutableElement toImplement, CssStylesheet cssStylesheet)
            throws UnableToCompleteException {
        ...
        assert toImplement.getReturnType().getKind().isPrimitive() || toImplement.getReturnType().getKind().toString().equals("java.lang.String");
        ...
        if (isReturnTypeString(toImplement.getReturnType())) {
            ...
            if (returnType == null) {
                logger.log(TreeLogger.ERROR, toImplement.getSimpleName()
            ...
            if (returnType.getKind() == TypeKind.INT || returnType.getKind() == TypeKind.LONG) {
                ...
                logger.log(TreeLogger.ERROR, returnType
     ...
     */
    private void writeClassAssignment(SourceWriter sw, ExecutableElement toImplement,
    ...
    private Map<ExecutableElement, String> optimize(TreeLogger logger,
                                                    ResourceContext context, ExecutableElement method) throws UnableToCompleteException {
        // Compute the local effective namespace
        TypeElement cssResourceSubtype = (TypeElement) MoreTypes.asElement(method.getReturnType());
        Map<String, Map<ExecutableElement, String>> classReplacementsWithPrefix = processImports(logger, cssResourceSubtype, method, context);
        ...
        Map<ExecutableElement, String> actualReplacements = new TreeMap<>(
     ...
     */
    private Map<String, Map<ExecutableElement, String>> processImports(TreeLogger logger,
                                                                       TypeElement cssResourceSubtype, ExecutableElement method,
        ...
        if (imp != null) {
            ...
            for (TypeMirror type : getImportype(imp)) {
                ...
                if (replacementsWithPrefix.put(prefix, computeReplacementsForType(importType, context)) != null) {
     ...
     * supertypes.
     */
    private Map<ExecutableElement, String> computeReplacementsForType(TypeElement type, ResourceContext context) {
    ...
    public void init(TreeLogger logger, ResourceContext context) throws UnableToCompleteException {
        ...
        SortedSet<TypeElement> cssResourceSubtypes = computeOperableTypes(logger, baseInterface, context.getClientBundleType(), context);
    ...
    private void computeObfuscatedNames(TreeLogger logger, String classPrefix,
        ...
        for (TypeElement type : cssResourceSubtypes) {
            ...
            for (Element method : MoreElements.getLocalAndInheritedMethods(type, types, elements)) {
                ...
                logger.log(TreeLogger.DEBUG, "Mapped " + type.getQualifiedName().toString()
     ...
     * TODO or scan return types ???
     */
    private SortedSet<TypeElement> computeOperableTypes(TreeLogger logger,
    ...
    private void ensureNoGssFile(URL[] resources, TreeLogger logger) throws UnableToCompleteException {
        for (URL stylesheet : resources) {
            if (stylesheet.getFile().endsWith(".gss")) {
                logger.log(TreeLogger.Type.ERROR, "GSS is not enabled. Add the following line to your gwt.xml file " +
                        "to enable it: " +
