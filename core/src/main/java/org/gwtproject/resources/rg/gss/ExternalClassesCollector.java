...
public class ExternalClassesCollector extends DefaultTreeVisitor implements CssCompilerPass {
  ...
  public void runPass() {
    ...
    remainingStyleClassNames = new HashSet<>();
    externalClassPrefixes = new ArrayList<>();
   ...
   */
  public ImmutableSet<String> getExternalClassNames(
      Set<String> styleClassesSet, Set<String> orphanClassName) {
    if (matchAll) {
    ...
    ImmutableSet.Builder<String> externalClassesSetBuilder = ImmutableSet.builder();
  ...
  private void processParameters(List<CssValueNode> values, SourceCodeLocation sourceCodeLocation) {
    for (CssValueNode value : values) {
      if (value instanceof CssCompositeValueNode) {
        ...
        errorManager.report(
            new GssError(
                "External at-rule invalid. The following terms is not "
                    + "accepted in an external at-rule ["
                    + value.getValue()
                    + "]",
                sourceCodeLocation));
      }
