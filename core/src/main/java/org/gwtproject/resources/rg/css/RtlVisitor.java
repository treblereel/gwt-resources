...
public class RtlVisitor extends CssModVisitor {
  /** Records if we're currently visiting a CssRule whose only selector is "body". */
  ...
  private void invokePropertyHandler(String name, List<Value> values) {
    ...
    try {
      ...
      try {
        ...
        Method m = getClass().getDeclaredMethod(methodName.toString(), Value.class);
      ...
      try {
        ...
        Method m = getClass().getDeclaredMethod(methodName.toString(), List.class);
  ...
  public boolean visit(CssRule x, Context ctx) {
    inBodyRule =
        x.getSelectors().size() == 1 && x.getSelectors().get(0).getSelector().equals("body");
    return true;
  }
  ...
  /**
   * Note there should be no propertyHandlerBorder(). The CSS spec states that the border property
   * must set all values at once.
   */
