...
import org.gwtproject.resources.rg.gss.*;
...
public class GssResourceGenerator extends AbstractCssResourceGenerator {
    ...
    public static GssOptions getGssOptions(ResourceContext context, TreeLogger logger) throws UnableToCompleteException {
        PropertyOracle propertyOracle = context.getGeneratorContext().getPropertyOracle();
        boolean gssEnabled = propertyOracle.getConfigurationProperty(logger, KEY_CSS_RESOURCE_ENABLE_GSS).asSingleBooleanValue();
        boolean gssDefaultInUiBinder = propertyOracle.getConfigurationProperty(logger, KEY_GSS_DEFAULT_IN_UIBINDER).asSingleBooleanValue();
        AutoConversionMode conversionMode = Enum.valueOf(AutoConversionMode.class,
                propertyOracle.getConfigurationProperty(logger, KEY_CSS_RESOURCE_CONVERSION_MODE).asSingleValue().toUpperCase(Locale.ROOT));
    ...
    @Override
    public void prepare(TreeLogger logger, ResourceContext context, ExecutableElement method) throws UnableToCompleteException {
    ...
    private Set<String> getTrueConfigurationProperties(ResourceContext context,
        ...
        for (String property : configurationProperties) {
            ...
            if (!"true".equals(confProp.getCurrentValue()) && !"false".equals(confProp.getCurrentValue())) {
                logger.log(Type.ERROR, "The eval property " + confProp.getName() + " is used in " +
                        "a conditional css. Its value must be either \"true\" or \"false\"");
                throw new UnableToCompleteException();
            }
    ...
    private void validateExternalClasses(Set<String> externalClasses,
                                         Set<String> externalClassCandidates, ExecutableElement method,
                                         TreeLogger logger) throws UnableToCompleteException {
    ...
    private RenamingResult doClassRenaming(CssTree cssTree, ExecutableElement method, TreeLogger logger,
    ...
    private Map<String, Map<String, String>> computeReplacements(ExecutableElement method, TreeLogger logger,
                                                                 ResourceContext context) throws UnableToCompleteException {
        ...
        replacementsWithPrefix
                .put("", computeReplacementsForType((TypeElement) MoreTypes.asElement(method.getReturnType()),
        ...
        if (imp != null) {
            ...
            for (TypeMirror type : getImportType(imp)) {
                ...
                if (replacementsWithPrefix.put(prefix, computeReplacementsForType(importType, context.getGeneratorContext().getAptContext())) != null) {
    ...
    private Map<String, String> computeReplacementsForType(TypeElement cssResource, AptContext aptContext) {
        ...
        if (replacements == null) {
            ...
            for (ExecutableElement method : MoreElements.getLocalAndInheritedMethods(cssResource, types, elements)) {
                ...
                if (replacementsForSharedMethods.containsKey(method)) {
                    ...
                    if (hasSharedAnnotation(method)) {
                        ...
                        replacement = obfuscationStyle.getPrettyName(styleClass, (TypeElement) method.getEnclosingElement(),
                                obfuscatedClassName);
    ...
    private Map<ExecutableElement, String> writeMethods(TreeLogger logger, ResourceContext context, ExecutableElement method,
        ...
        for (ExecutableElement toImplement : MoreElements.getLocalAndInheritedMethods(gssResource, types, elements)) {
    ...
    private boolean writeUserMethod(TreeLogger logger, ExecutableElement userMethod, SourceWriter sw,
                                    ConstantDefinitions constantDefinitions, Map<String, String> originalConstantNameMapping,
                                    Map<String, String> substitutionMap, Map<ExecutableElement, String> methodToClassName)
            throws UnableToCompleteException {

        String className = getClassName(userMethod);
        // method to access style class ?
        if (substitutionMap.containsKey(className) &&
                isReturnTypeString(userMethod.getReturnType())) {
        ...
        if (substitutionMap.containsKey(className)) {
            ...
            logger.log(Type.ERROR, "The return type of the method [" + userMethod.toString() + "] must " +
                    "be java.lang.String.");
            throw new UnableToCompleteException();
        }
        ...
        logger.log(Type.ERROR,
                "The following method [" + userMethod.toString() + "] doesn't match a constant" +
    ...
    private boolean writeClassMethod(TreeLogger logger, ExecutableElement userMethod,
                                     Map<String, String> substitutionMap, SourceWriter sw) throws
            UnableToCompleteException {

        if (userMethod.getParameters().size() > 0) {
            logger.log(Type.ERROR, "The method [" + userMethod.toString() + "] shouldn't contain any " +
                    "parameters");
            throw new UnableToCompleteException();
        }
    ...
    protected boolean isReturnTypeString(TypeMirror classReturnType) {
        return (classReturnType != null && !classReturnType.getKind().isPrimitive()
    ...
    private boolean writeDefMethod(CssDefinitionNode definitionNode, TreeLogger logger,
                                   ExecutableElement userMethod, SourceWriter sw) throws UnableToCompleteException {
        ...
        if (params.size() != 1 && !isReturnTypeString(classReturnType)) {
            logger.log(ERROR, "@def rule " + name
                    + " must define exactly one value or return type must be String");
            return false;
        }
        assert classReturnType.getKind().isPrimitive() || classReturnType.getKind().toString().equals("java.lang.String");
        ...
        if (isReturnTypeString(classReturnType)) {
            ...
            if (returnType == null) {
                logger.log(ERROR, name + ": Return type must be primitive type " +
            ...
            if (!(valueNode instanceof CssNumericNode)) {
                logger.log(ERROR, "The value of the constant defined by @" + name + " is not a" +
                        " numeric");
                return false;
            }
    ...
    public void init(TreeLogger logger, ResourceContext context) throws UnableToCompleteException {
        ...
        obfuscationStyle = CssObfuscationStyle
                .getObfuscationStyle(propertyOracle
                        .getConfigurationProperty(logger, KEY_CSS_RESOURCE_STYLE).asSingleValue());
        obfuscationPrefix = getObfuscationPrefix(logger, propertyOracle
                .getConfigurationProperty(logger, KEY_CSS_RESOURCE_OBFUSCATION_PREFIX).asSingleValue(), context);
        allowedAtRules.addAll(propertyOracle
                .getConfigurationProperty(logger, KEY_CSS_RESOURCE_ALLOWED_AT_RULES).getValues());
        allowedNonStandardFunctions.addAll(propertyOracle
                .getConfigurationProperty(logger, KEY_CSS_RESOURCE_ALLOWED_FUNCTIONS).getValues());
    ...
    private SortedSet<TypeElement> computeOperableTypes(TreeLogger logger, ResourceContext context) {
        ...
        SortedSet<TypeElement> toReturn = new TreeSet<>(new CssResourceGenerator.JClassOrderComparator());
    ...
    private void initReplacement(ResourceContext context) {
        if (context.getCachedData(KEY_HAS_CACHED_DATA, Boolean.class) != Boolean.TRUE) {
            ...
            context.putCachedData(KEY_BY_CLASS_AND_METHOD, new IdentityHashMap<TypeElement, Map<String,
                    String>>());
