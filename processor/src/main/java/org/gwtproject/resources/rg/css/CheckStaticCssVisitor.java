...
public class CheckStaticCssVisitor extends CssVisitor {
  ...
  /**
   * Returns <code>true</code> if the stylsheet is statically-evaluable. Nodes that are not
   * statically-evaluable will be reported to the associated logger.
   */
  ...
  private CheckStaticCssVisitor(TreeLogger logger, boolean fastFail) {
    this.logger =
        logger.branch(TreeLogger.DEBUG, "Checking external stylesheet for dynamic content");
  ...
  void error(CssNode node) {
    logger.log(
        TreeLogger.ERROR, "The CSS node " + node.toString() + " cannot be statically evaluated");
    error = true;
  }
