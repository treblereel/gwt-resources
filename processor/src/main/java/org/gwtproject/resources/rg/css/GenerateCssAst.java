...
public class GenerateCssAst {
  ...
  /**
   * Maps SAC CSSParseExceptions into a TreeLogger. All parsing errors will be recorded in a single
   * TreeLogger branch, which will be created only if a loggable error message is emitted.
   */
  private static class Errors implements ErrorHandler {
     ...
     * Constructor.
     *
     * @param parentLogger the TreeLogger that should be branched to produce the CSS parsing
     *     messages.
     */
    ...
    private void log(TreeLogger.Type type, CSSParseException e) {
      log(
          type,
          "Line " + e.getLineNumber() + " column " + e.getColumnNumber() + ": " + e.getMessage());
    }

    private TreeLogger logOrBranch(
        TreeLogger.Type type, String message, Throwable t, boolean branch) {
    ...
    private void maybeBranch() {
      if (logger == null) {
        logger = parentLogger.branch(TreeLogger.INFO, "The following problems were detected");
      }
    }
  }
  ...
  private static class GenerationHandler implements DocumentHandler {
    ...
    /**
     * Used by {@link #startSelector(SelectorList)} to suppress the creation of new CssRules in
     * favor of retaining {@link #currentRule}.
     */
    ...
    public void ignorableAtRule(String atRule) throws CSSException {
      ...
      String methodName =
          "parse"
              + (Character.toUpperCase(ruleName.charAt(0)))
      ...
      try {
        ...
        if (cause instanceof CSSException) {
          ...
          TreeLogger details =
              errors.branch(TreeLogger.ERROR, "Unknown failure parsing " + ruleName);
    ...
    public void property(String name, LexicalUnit value, boolean important) throws CSSException {
      ...
      currentRule
          .getProperties()
          .add(new CssProperty(escapeIdent(name), new ListValue(values), important));
    }
    ...
    /**
     * The elif nodes are processed as though they were {@code @if} nodes. The newly-generated CssIf
     * node will be attached to the last CssIf in the if/else chain.
     */
    void parseElif(String atRule) throws CSSException {
      ...
      if (lastIf == null) {
        throw new CSSException(
            CSSException.SAC_SYNTAX_ERR, "@elif must immediately follow an @if or @elif", null);
      }
    ...
    /** The else nodes are processed as though they were written as {@code @elif true} rules. */
    void parseElse(String atRule) throws CSSException {
      ...
      if (lastIf == null) {
        throw new CSSException(
            CSSException.SAC_SYNTAX_ERR, "@else must immediately follow an @if or @elif", null);
      }
    ...
    void parseEval(String atRule) throws CSSException {
      ...
      if (parts.length != 3) {
        throw new CSSException(
            CSSException.SAC_SYNTAX_ERR, "Incorrect number of parts for @eval", null);
      }
    ...
    void parseExternal(String atRule) throws CSSException {
      ...
      String[] parts =
          atRule
              .substring("@external ".length(), atRule.length() - 1)
              .replaceAll("(,|\\s)\\s*", " ")
              .replaceAll("\\.", "")
              .split(" ");
    ...
    void parseIf(String atRule) throws CSSException {
      ...
      if (predicate.startsWith("(") && predicate.endsWith(")")) {
        ...
        switch (predicateParts.length) {
          case 0:
            throw new CSSException(
                CSSException.SAC_SYNTAX_ERR, "Incorrect format for @if predicate", null);
          case 1:
    ...
    void parseSprite(String atRule) throws CSSException {
      ...
      try {
        ...
        throw new CSSException(CSSException.SAC_SYNTAX_ERR, "Unable to parse @sprite", e);
      }
    }
    ...
    void parseUrl(String atRule) throws CSSException {
      ...
      if (parts.length != 3) {
        throw new CSSException(
            CSSException.SAC_SYNTAX_ERR, "Incorrect number of parts for @url", null);
      }
    ...
    /** Adds a node to the current parent and then makes the node the current parent node. */
  ...
  /**
   * We cache the stylesheets to prevent repeated parsing of the same source material. This is a
   * common case if the user is using UiBinder's implicit stylesheets. It is necessary to use the
   * list of URLs passed to exec because of the eager variable expansion performed by {@link
   * GenerationHandler#parseDef(String)}.
   */
  private static final Map<List<URL>, SoftReference<CachedStylesheet>> SHEETS =
      Collections.synchronizedMap(new HashMap<List<URL>, SoftReference<CachedStylesheet>>());

  private static final String VALUE_FUNCTION_NAME = "value";

  /**
   * Create a CssStylesheet from the contents of one or more URLs. If multiple URLs are provided,
   * the generated stylesheet will be created as though the contents of the URLs had been
   * concatenated.
   */
  public static CssStylesheet exec(TreeLogger logger, URL... stylesheets)
    ...
    for (URL stylesheet : sheets) {
      TreeLogger branchLogger =
          logger.branch(TreeLogger.DEBUG, "Parsing CSS stylesheet " + stylesheet.toExternalForm());
      try {
   ...
   *
   * @param colors a sequence of LexicalUnits, assumed to be <code>(VAL COMMA VAL COMMA VAL)</code>
  ...
  /**
   * The elif and else constructs are modeled as nested if statements in the CssIf's elseNodes
   * field. This method will search a list of CssNodes and remove the last chained CssIf from the
   * last element in the list of nodes.
   */
   ...
   *
   * @param color typed value from the CSS parser, which may be an INTEGER or a PERCENTAGE
   * @return integer value from 0-255
   * @throws IllegalArgumentException if the color is not an INTEGER or PERCENTAGE value
   */
      ...
      case LexicalUnit.SAC_FUNCTION:
        {
          if (value.getFunctionName().equals(VALUE_FUNCTION_NAME)) {
            ...
            String suffix =
                params.size() == 3 ? maybeUnquote(((StringValue) params.get(2)).getValue()) : "";
            ...
            if (!(expression instanceof StringValue)) {
              throw new CSSException(
                  CSSException.SAC_SYNTAX_ERR,
                  "The single argument to " + LITERAL_FUNCTION_NAME + " must be a string value",
                  null);
            }
