 ...
 * constants, create the GSS compatible name of each constant and returns a mapping of all original
 * names with the new generated name.
 *
 * <p>This visitor lists also all constant nodes.
 */
  ...
  private boolean handleDefNode(CssDef def) {
    ...
    if (!defMapping.containsKey(def.getKey())) {
      ...
      if (!validName.equals(upperCaseName)) {
        treeLogger.log(
            Type.WARN,
            "Invalid characters detected in ["
                + upperCaseName
                + "]. They have "
                + "been replaced ["
                + validName
      ...
      if (defMapping.containsValue(validName)) {
        if (lenient) {
          ...
          throw new Css2GssConversionException(
              "Two constants have the same name [" + validName + "] after conversion.");
        }
      }
