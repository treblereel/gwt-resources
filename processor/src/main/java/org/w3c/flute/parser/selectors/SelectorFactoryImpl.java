...
public class SelectorFactoryImpl implements SelectorFactory {
   ...
   * @param tagName the <a href="http://www.w3.org/TR/REC-xml-names/#NT-LocalPart">local part</a> of
   *     the element name. <code>NULL</code> if this element selector can match any element.
   ...
   *
   * @param pseudoName the pseudo element name. <code>NULL</code> if this element selector can match
   *     any pseudo element.
   * @return the element selector
   * @exception CSSException If this selector is not supported.
   */
  ...
  public SiblingSelector createDirectAdjacentSelector(
      short nodeType, Selector child, SimpleSelector directAdjacent) throws CSSException {
    if (nodeType != 1) {
      throw new CSSException(CSSException.SAC_NOT_SUPPORTED_ERR);
    } else {
