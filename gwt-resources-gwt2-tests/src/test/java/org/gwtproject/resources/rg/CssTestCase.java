...
/**
 * Contains functions for golden-output tests that are concerned with structural modifications to
 * the CSS AST.
 */
@Ignore
public class CssTestCase extends TestCase {
  ...
  public static class AliasDetector extends CssVisitor {
    ...
    protected <T extends CssNode> T doAccept(T node) {
      assertFalse("Found repeated node " + node.toString(), seen.containsKey(node));
      seen.put(node, null);
  ...
  private static class FakeContext implements ResourceContext {
    ...
    @Override
    public String getImplementationSimpleSourceName() throws IllegalStateException {
      return null;
    }
  ...
  protected static <T extends CssNode & HasNodes> void assertEquals(
    ...
    String expectedCss =
        CssResourceGenerator.makeExpression(logger, new FakeContext(), expected, false);
    String testCss = CssResourceGenerator.makeExpression(logger, new FakeContext(), test, false);
  ...
  /**
   * Compares the generated Java expressions for an input file, transformed in order by the
   * specified visitors, and a golden-output file.
   */
  private static void test(TreeLogger logger, URL test, URL expected, CssVisitor... visitors)
   ...
   * @param reversible if <code>true</code>, the test will attempt to transform the expected css
   *     into the test css
   */
  protected void test(
      TreeLogger logger, String testName, boolean reversible, CssVisitor... visitors)
      throws UnableToCompleteException {
    String packagePath = getClass().getPackage().getName().replace('.', '/') + "/";
    URL testUrl = getClass().getClassLoader().getResource(packagePath + testName + "_test.css");
    assertNotNull("Could not find testUrl", testUrl);
    URL expectedUrl =
        getClass().getClassLoader().getResource(packagePath + testName + "_expected.css");
